...
class Images4Up extends s {
    ...
    render() {
        const { x, y: y$1, width, height } = this.containerRect;
        return y `
<style>
:host {
  display: block;
...
<div id="container"
    ...
    style="transform: translate(${x}px, ${y$1}px);
           width: ${width}px;
           height: ${height}px">
...
<button @click="${() => this.reset()}" class="no-border">
  Clear selections
            ...
            instructions = y `<p>Select two images to compare them!</p>`;
        }
        return y `
<style>
:host {
...
class RenderingScenario extends s {
    ...
    metricTemplate(analysisResults, goldenName) {
        ...
        return y ` <span>${rmsInDecibel} dB </span>
      <div class="tooltip">
...
class ImageComparisonApp extends s {
    ...
    render() {
        const { config } = this;
        if (config == null) {
            return this.src ? y `Loading...` : y `No config specified`;
        }
        const goldens = config.renderers.map(renderer => ({ ...renderer, file: `${renderer.name}-golden.png` }));
        const scenarios = config.scenarios.map((scenario) => y `
